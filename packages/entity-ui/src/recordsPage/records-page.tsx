...
export interface EntityRecordsPageProps {
  ...
  gridSettings?: Partial<EntityDataGridSettings>;
  ...
  return (
    <section className="system-designer-page__section system-designer-records-page" aria-label={labels.recordsTitle}>
      <div className="system-designer-records-page__toolbar">
        <h2>{title}</h2>
        <Button unstyled className="titanic-records-page__button titanic-records-page__button_primary" type="button" onClick={startCreate}>
          {labels.createRecord}
        </Button>
      </div>
          ...
          <section className="titanic-record-modal__card">
            <header className="titanic-record-modal__header">
              ...
              <div className="titanic-record-modal__header-actions">
                ...
                <Button unstyled className="titanic-records-page__button titanic-records-page__button_secondary" type="button" onClick={cancelEdit}>
  ...
  return (
    <section className="titanic-record-form-designer">
      <div className="titanic-record-form-designer__heading">
        ...
        <Button unstyled className="titanic-records-page__button titanic-records-page__button_primary" type="button" disabled={disabled} onClick={onSave}>
      ...
      <div className="titanic-record-form-designer__layout">
              ...
              return (
                ...
                >
                  <div className="titanic-record-form-designer__field-head">
                    ...
                    <Button unstyled type="button" disabled={disabled} onClick={() => hideColumn(column.path)}>
        ...
        <aside className="titanic-record-form-designer__panel titanic-record-form-designer__available">
          ...
          {hiddenColumns.length > 0 ? hiddenColumns.map((column) => (
            <Button unstyled type="button" key={column.path} disabled={disabled} onClick={() => showColumn(column.path)}>
...
function normalizeColumns(columns: readonly EntityColumnDefinition[]): EntityColumnSchema[] {
  const normalizedColumns = columns.map((column) => normalizeEntityColumn(column));
  const hasExplicitOrder = normalizedColumns.some((column) => typeof (column as { order?: unknown }).order === "number");

  return [...normalizedColumns].sort((left, right) => {
    if (hasExplicitOrder) {
...
function getRecordLoadColumnPaths(schema: EntitySchema): string[] {
  return [...new Set(schema.columns.map((column) => normalizeEntityColumn(column).path))];
}

export const recordsPageComponentSchema = defineComponentSchema<EntityRecordsPageProps>({
  kind: "component",
